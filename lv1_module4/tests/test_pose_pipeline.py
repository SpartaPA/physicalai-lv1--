...
def test_camera_to_base_matches_chain(pipeline, rng):
    ...
    chain = default_chain()
    P_base = pipeline.camera_to_base(P)
    T = chain.get("base", "link") @ chain.get("link", "camera")
    assert P_base.shape == P.shape
    assert np.allclose(P_base, chain.transform("base", "camera", P))
    assert np.allclose(P_base, transform_points(T, P))


# --- 2. 왕복 검증 -------------------------------------------------------------

def test_roundtrip_restores_points(pipeline, rng):
    P_cam = rng.normal(size=(200, 3))
    for points in (P_cam, P_cam[0]):
        restored = pipeline.base_to_camera(pipeline.camera_to_base(points))
