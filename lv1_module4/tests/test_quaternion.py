...
def test_slerp_is_unit_norm(q_pair, t):
    assert np.isclose(np.linalg.norm(slerp(*q_pair, t)), 1.0)
...
def test_slerp_endpoints(q_pair):
    q0, q1 = q_pair
    for t, expected in ((0.0, q0), (1.0, q1)):
        assert np.allclose(quaternion_to_matrix(slerp(q0, q1, t)), quaternion_to_matrix(expected))
