...
class PosePipeline:
    ...
    def __init__(self, T_base_link, T_link_camera, joint_axis: str = "z"):
        ...
        if np.asarray(T_base_link).shape == (4, 4) and np.asarray(T_link_camera).shape == (4, 4):
            self._T_base_link0 = np.asarray(T_base_link)
            self._T_link_camera = np.asarray(T_link_camera)
        else:
    ...
    def T_base_link(self) -> np.ndarray:
        ...
        """
        # TODO: 문제 2-2
        T_base_link = self._T_base_link0 @ make_T(_ROT[self.joint_axis](self.joint_angle), [0, 0, 0])
        return T_base_link
